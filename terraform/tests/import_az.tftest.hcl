run "test_import_az_function" {
  ...
  variables {
    function_name = "cmmxna-pr-import_azure"
    payload = jsonencode({
  ...
  assert {
    ...
    error_message = "Import Azure failed unexpectedly. Status: ${output.status_code}. Messages ${output.error_messages}."
  }
}